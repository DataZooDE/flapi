...
class TestDuckLakeAdvanced:
    ...
    def test_ducklake_snapshot_management(self, api_config_url):
        ...
        for i in range(3):
            response = requests.post(f"{api_config_url}/endpoints/test/cached/cache/refresh")
            assert response.status_code == 200
            time.sleep(1)  # Small delay between refreshes
        
        # Check audit log for snapshot information
        audit_response = requests.get(f"{api_config_url}/endpoints/test/cached/cache/audit")
    ...
    def test_cache_mode_determination(self, api_config_url):
        """Test that cache modes are correctly determined based on configuration"""
        
        # Test full refresh mode (no cursor, no primary key)
        full_response = requests.get(f"{api_config_url}/endpoints/test/cached/cache")
        ...
        append_response = requests.get(f"{api_config_url}/endpoints/test/append/cache")
        ...
        merge_response = requests.get(f"{api_config_url}/endpoints/test/merge/cache")
    ...
    def test_retention_policy_execution(self, api_config_url):
        ...
        audit_response = requests.get(f"{api_config_url}/cache/audit")
        assert audit_response.status_code == 200
    ...
    def test_scheduled_cache_refresh(self, api_config_url):
        """Test that scheduled cache refreshes work correctly"""
        # Get cache configuration to verify scheduling is enabled
        response = requests.get(f"{api_config_url}/endpoints/test/cached/cache")
    ...
    def test_ducklake_compaction(self, api_config_url):
    ...
    def test_cache_template_processing(self, api_config_url):
    ...
    def test_ducklake_catalog_attachment(self, api_config_url):
        """Test that DuckLake catalog is properly attached"""
        # Check DuckLake configuration
        response = requests.get(f"{api_config_url}/ducklake")
    ...
    def test_error_handling(self, api_config_url):
        """Test error handling in cache operations"""
        # Test with non-existent endpoint
        response = requests.get(f"{api_config_url}/endpoints/nonexistent/cache")
        assert response.status_code == 404
        
        # Test cache refresh on non-existent endpoint
        response = requests.post(f"{api_config_url}/endpoints/nonexistent/cache/refresh")
        assert response.status_code == 404
    
    def test_concurrent_cache_operations(self, api_config_url):
    ...
    def test_cache_performance(self, api_config_url):
